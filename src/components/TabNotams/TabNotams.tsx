...
function TabNotams() {
    const { setNotamCoords, resetNotamCoords, setAreaCoords, setIsOpenModal, reservationsCoords, setReservationCoords, deleteReservationCoord } = useStore();
    ...
    const handleChange = (checked: boolean, obj: IArea) => {
        if (checked) {
            setReservationCoords(obj)
        } else {
            deleteReservationCoord(obj.id)
        }
    }
                ...
                {!isFilter && output
                    ...
                    .map(obj => (
                        <div
                            ...
                            <input type='checkbox' checked={Boolean(reservationsCoords.find(item => item.id === obj.id))} onChange={(e) => handleChange(e.target.checked, obj)} />
